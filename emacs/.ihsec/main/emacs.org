   ...
   #+begin_src emacs-lisp
     (use-package emacs
       ...
       (use-package doom-modeline
         :custom
         (doom-modeline-window-width-limit 207)
...
*** Setup
    ...
    - Use =search= and =searchadd= commands instead of =find= and =findadd= - fixes albums appearing empty when opened, which I believe is because =find= commands, when receive an empty string, look for files with that content, in this case, empty, while =search= commands treat the empty string as a wildcard.
