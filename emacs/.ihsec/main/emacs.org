...
** Embark
   ...
   Among it's most interesting action, in my opinion, is "export",
...
** Prescient
   A package for filtering and sorting completion candidates. I don't
   want its filtering to interfere with my completion styles, but the
   sorting (based on frequency and recency, like =fasd=) is really
   neat. This package is the library that is meant to be used by its
   implementations for other packages such as =company=, so I'll set
   that in the appropriate section. Here I load the =prescient= library
   and activate =prescient-persist-mode=, which will make the stats for
   sorting persist through Emacs sessions.

   [[https://github.com/raxod502/prescient.el][Git repository]]
   #+begin_src emacs-lisp
...
** Company
   ...
   #+begin_src emacs-lisp
     (use-package company
       ...
       ;; Prescient ordering of candidates.
