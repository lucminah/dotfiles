    ...
    #+begin_src emacs-lisp
      (use-package corfu
        ...
        (load-file (concat user-emacs-directory
                           "straight/repos/corfu/extensions/corfu-popupinfo.el"))
        (setq corfu-popupinfo-delay (cons 0.5 0))
...
** Fasd
   ...
   [[https://github.com/lucminah/fasd][My fork]]
   #+begin_src emacs-lisp
     (use-package fasd
       :straight (fasd :type git :host github :repo "emacsmirror/fasd"
                       :fork (:type git :repo "lucminah/fasd"))
       :init
...
** Golden Ratio
   Resizes windows based on the golden ratio. Makes the currently
   focused window the biggest and adjusts very well to different
   layouts. Window management just got so much easier, this is a must
   have for me now.

   [[https://github.com/roman/golden-ratio.el][Git repository]]
   #+begin_src emacs-lisp
     (use-package golden-ratio
       :init
       (setq golden-ratio-auto-scale nil
             golden-ratio-extra-commands '(evil-window-delete
...
** Focus Mode
   Very useful package for keeping only the current text block
   highlighted, dimming the rest of the buffer.

   [[https://github.com/larstvei/Focus][Git repository]]
   #+begin_src emacs-lisp
     (use-package focus
       :init
       (luc/leader-def
...
** Scroll on Jump
   Interesting package that smooths out the moving buffer when jumping
   around. It's not perfect, doesn't work exactly as I'd like it to,
   but I'm enjoying it.

   [[https://github.com/emacsmirror/scroll-on-jump][Git repository]]
   #+begin_src emacs-lisp
...
** Evil Split
   My preferred behavior for splitting windows.
...
*** Setup
    ...
    [[https://github.com/lucminah/libmpdel][My fork of libmpdel]]

    [[https://github.com/lucminah/mpdel][My fork of mpdel]]
    #+begin_src emacs-lisp
      (use-package libmpdel
        :straight (libmpdel :type git :host github :repo "mpdel/libmpdel"
                            :fork (:host github :repo "lucminah/libmpdel"))
        :config
        (use-package mpdel
          :straight (mpdel :type git :host github :repo "mpdel/mpdel"
                           :fork (:host github :repo "lucminah/mpdel"))
...
*** Vterm
    ...
    I'm also using =multi-vterm= so I can have multiple instances of it
    running at the same time.

    [[https://github.com/akermu/emacs-libvterm][Vterm]] | [[https://github.com/suonlight/multi-vterm][Multi-Vterm]]
    #+begin_src emacs-lisp
      (use-package vterm
        :init
...
**** General configuration
     I use a package to highlight quoted symbols and =lispyville=, that
     provides easier Lisp editing, mainly by keeping parenthesis
     balanced. Currently, I'm not using =lispy= in conjunction with it
     because it really messed up my keybindings, but I'll check it out
     again sometime, and keeping the link to the repo here for future
     reference.

     [[https://github.com/Fanael/highlight-quoted][Highlight Quoted]] | [[https://github.com/abo-abo/lispy][Lispy]] | [[https://github.com/noctuid/lispyville][Lispyville]]
     ...
     #+begin_src emacs-lisp
       (use-package sly
         ...
         (general-def
           ...
           :states '(normal insert)
           "C-<return>" 'sly-compile-defun
           "S-<return>" 'sly-eval-last-expression
           "C-S-<return>" 'sly-eval-defun)
       ...
       (use-package geiser
         ...
         (use-package geiser-guile
           ...
           (general-def
             ...
             :states '(normal insert)
             "C-<return>" 'geiser-compile-definition
             "S-<return>" 'geiser-eval-last-sexp
             "C-S-<return>" 'geiser-eval-definition)))
     #+end_src
...
**** Clojure
     #+begin_src emacs-lisp
       (use-package clojure-mode
         ...
         (clojure-mode . (lambda () (setq-local lsp-completion-enable nil)))
         :init
         (use-package cider
           :hook
           (clojure-mode . cider-mode)
           :init
           (defun luc/reload-rainbow-identifiers ()
             (interactive)
             (progn
               (rainbow-identifiers-mode -1)
               (rainbow-identifiers-mode 1)))
           (advice-add 'cider-eval-defun-at-point :after #'luc/reload-rainbow-identifiers)
           (general-def
             :keymaps 'cider-mode-map
             :states '(normal insert)
             "C-<return>" 'cider-eval-defun-at-point
             "S-<return>" 'cider-eval-last-sexp
             "C-S-<return>" 'cider-eval-last-sexp-to-repl)))
     #+end_src
